...
class Env_net(nn.Module):
    def __init__(self, meteo_num):
        ...
        self.data_embed['location_long'] = nn.Linear(9, embed_dim)
        self.data_embed['location_lat'] = nn.Linear(7, embed_dim)
        self.data_embed['history_direction12'] = nn.Linear(8, embed_dim)
        self.data_embed['history_direction24'] = nn.Linear(8, embed_dim)
        self.data_embed['history_intensity_change24'] = nn.Linear(4, embed_dim)

        self.GPH_embed = nn.Sequential(
            nn.Conv2d(meteo_num, 1, kernel_size=(3,3), stride=(1,1), padding=(1,1), bias=True),
            nn.BatchNorm2d(1),
    ...
    def forward(self,inner_data,meteo):
        '''

        :param inner_data: b,x_len
        :param meteo: b,1,h,w  （b,c,h,w）
        :return:
        '''
        # print(meteo) 
        batch = meteo.shape[0]
        ...
        for key in self.data_embed:
            now_embed = self.data_embed[key](inner_data[key])
            embed_list.append(now_embed)
        embed_list.append(self.GPH_embed(meteo).reshape(batch,-1))
