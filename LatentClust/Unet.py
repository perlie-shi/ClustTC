...
class Conv3d(nn.Module):
    def __init__(self, in_channel, out_channel, kernel_size, stride, padding):
        ...
        self.conv1 = nn.Sequential(#in_channel=1, out_channel=16
...
class Unet3D(nn.Module):
    def __init__(self, in_channel, out_channel): #in_channel=meteo_num, out_channel=1
    ...
    def forward(self, x):
        ...
        x1 = self.inc(x)  # [96, 16, 8, 64, 64]
        # output = self.h2input(output)
        # x1 = x1 + output.permute(1, 2, 0).contiguous().unsqueeze(-1).unsqueeze(-1)
        x2 = self.down1(x1)     # [96, 32, 8, 32, 32]
        x3 = self.down2(x2)     # [96, 64, 8, 16, 16]
        x4 = self.down3(x3)     # [96, 128, 4, 8, 8]
        x5 = self.down4(x4)     # [96, 128, 2, 4, 4]

        # final_encoder_h = self.h2b(final_encoder_h.view(batch, -1))  # [batch,512]
        # x5 = x5 + final_encoder_h.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)

        x6 = self.up1(x5, x4)     # [96, 64, 4, 8, 8]
        x7 = self.up2(x6, x3)     # [96, 32, 8, 16, 16]
        x8 = self.up3(x7, x2)     # [96, 16, 8, 32, 32]
        x9 = self.up4(x8, x1)     # [96, 16, 8, 64, 64]

        out = self.outc([x6,x7,x8,x9])      # [96, 1, 11, 64, 64]
        # fusion_feature = self.x52fusion(x5.view(batch, -1))
        return out

if __name__ == '__main__':

    x = torch.randn((96, 11, 8, 64, 64)).cuda()  #
    # final_encoder_h = torch.randn((1, 1, 64)).cuda()
    # output = torch.randn((8, 1, 64)).cuda()
    net = Unet3D(11, 11).cuda()
    out = net(x)
    print(out.shape)
